...
删除并非立即销毁：文件会移动到其所属存储卷根目录下的 `.ane-filemanager-trash`，避免删除可移除存储上的大文件时退化为跨卷复制。一次批量删除可以跨多个卷，但对每个文件始终选择同卷回收目录。`FileHistoryController` 保存带父子关系的双向动作；撤回后执行新操作会保留原分支，并可重做到最新子分支或按节点切换分支。记录仍只保存在当前进程会话内，没有固定步数上限；重新启动应用时会清理各已挂载存储卷中的旧回收内容，因此历史不是跨进程持久化功能。
