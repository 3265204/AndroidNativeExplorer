...
internal data class ImagePalette(
    val dark: Boolean, val background: Int, val surface: Int, val text: Int,
    val muted: Int, val primary: Int, val divider: Int
) {
    companion object {
        fun from(context: Context): ImagePalette {
            val dark = context.getSharedPreferences("appearance", Context.MODE_PRIVATE).getBoolean("dark", false)
            return if (dark) ImagePalette(true, Color.rgb(18, 22, 29), Color.rgb(28, 34, 43),
                Color.rgb(235, 240, 247), Color.rgb(160, 171, 186), Color.rgb(59, 130, 246), Color.rgb(52, 63, 77))
            else ImagePalette(false, Color.rgb(248, 250, 252), Color.WHITE, Color.rgb(25, 33, 45),
                Color.rgb(100, 116, 139), Color.rgb(59, 130, 246), Color.rgb(226, 232, 240))
        }
    }
}

internal fun Activity.applyImageSystemBars(palette: ImagePalette) = applyImageBars(palette.surface, palette.background, palette.dark)

private fun Activity.applyImageBars(status: Int, navigation: Int, dark: Boolean) {
    ...
    if (Build.VERSION.SDK_INT >= 30) {
        ...
        flags = if (!dark) flags or View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR
        else flags and View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR.inv()
        if (Build.VERSION.SDK_INT >= 26) {
            flags = if (!dark) flags or View.SYSTEM_UI_FLAG_LIGHT_NAVIGATION_BAR
...
internal fun View.applyImageSystemInsets() = applyImageInsets()

private fun View.applyImageInsets() {
