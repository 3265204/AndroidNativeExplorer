package com.ane.filemanager.viewer.audio
...
class AudioViewerActivity : Activity() {
    ...
    override fun onCreate(state: Bundle?) {
        ...
        playlist = MediaFolderPlaylist.create(file, MediaKind.AUDIO)
        ...
        elapsed = TextView(this).apply { text = "0:00"; setTextColor(palette.muted); textSize = 12f }
        durationLabel = TextView(this).apply { text = "0:00"; setTextColor(palette.muted); textSize = 12f; gravity = Gravity.END }
        ...
        previousButton = audioSwitchButton("‹", R.string.viewer_previous, palette).apply {
            setOnClickListener { switchAudio(-1) }
        }
        nextButton = audioSwitchButton("›", R.string.viewer_next, palette).apply {
            setOnClickListener { switchAudio(1) }
        }
    ...
    private fun audioSwitchButton(symbol: String, description: Int, palette: ViewerPalette) = Button(this).apply {
        text = symbol
        textSize = 34f
        setTextColor(palette.text)
        setBackgroundColor(Color.TRANSPARENT)
    ...
    private fun preparePlayer(file: File) {
        player = try { MediaPlayer().apply {
            ...
            setOnErrorListener { _, _, _ ->
                Toast.makeText(this@AudioViewerActivity, R.string.viewer_audio_failed, Toast.LENGTH_SHORT).show()
                updatePlayButton(false)
                true
            }
            prepareAsync()
            ...
            player?.release()
            Toast.makeText(this, R.string.viewer_audio_failed, Toast.LENGTH_SHORT).show()
            null
        }
    }
